...
`test -e filetree || git clone -b pharo1.3 https://github.com/dalehenrich/filetree.git`
`git --git-dir=filetree/.git pull`

`test -e pharo-core || git clone --no-checkout git@github.com:PharoProject/pharo-core.git`
...
# find the last version in the git repository ==================================
print yellow("Moving git repository to ")

tags = `git --git-dir=pharo-core/.git tag --list`.split("\n").sort!
gitVersion = 0
nextGitVersion = 'HEAD'
overWrite = false

tags.each {|i|
  overWrite = true if VERSION == i.to_i
  if VERSION < i.to_i
    nextGitVersion = i
    break
  end
...
puts gitVersion
`git --git-dir='pharo-core/.git' reset #{gitVersion}`

# ==============================================================================
puts yellow("Using #{versionNumber} as base image ")

`wget #{versionFile}`
`unzip -o #{File.basename(versionFile)}`
`mv Pharo-*/*.image update.image`
`mv Pharo-*/*.changes update.changes`
...
FileStream stdout nextPutAll: 'Exporting Core...'; crlf.
!

IDENTIFIER
}

`cat pharo-build/scripts/pharo/pharo-2.0-git-tracker.st >> export.st`
...
#`$PHARO_VM -headless update.image export.st`

`touch pharo-core/#{VERSION}`
`echo #{VERSION} > #{VERSION}`

# commit ======================================================================
puts " Committing pharo-core version: #{VERSION}"
puts "========================================================"
puts `cat #{VERSION}`
puts "========================================================"


Dir.chdir 'pharo-core'
`git add *`
`git commit --file ../#{VERSION}`
`git tag #{VERSION}`

# patching the history =========================================================
print yellow("Rebasing #{nextGitVersion}")

`git checkout #{nextGitVersion}~0`
`git reset --soft #{VERSION}`
`git commit -C #{nextGitVersion} -- `
`git replace -d #{nextGitVersion} HEAD`
