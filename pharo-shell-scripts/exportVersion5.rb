...
system("test -e pharo-core || git clone --depth=1 -b 5.0 --no-checkout #{REPOS}")
...
if !ENV.has_key? 'PHARO_VM'
  ...
  system('wget --quiet -O - get.pharo.org/vm50 | bash')
...
exec("$PHARO_VM Pharo-#{PHARO_VERSION}.image #{SCRIPTS}/scripts/pharo/pharo-5.0-git-tracker.st")
